...
{
    internal class CancelBonusValidator : AbstractValidator<CancelBonusRedemption>
    {
        public CancelBonusValidator(IBonusRepository repository)
        {
            ...
            RuleFor(model => model)
                ...
                .Must(model => redemptionGetter(model.PlayerId, model.RedemptionId).RolloverState == RolloverStatus.Active)
                .WithMessage(ValidatorMessages.BonusRedemptionStatusIsIncorrectForCancellation)
                .WithName("Model");
        }
    }
}