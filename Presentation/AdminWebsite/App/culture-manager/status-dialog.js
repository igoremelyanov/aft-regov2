define(['i18next', 'plugins/dialog'], function (i18n, dialog) {
    ...
    var customModal = function (code, originalStatus, callback) {
        ...
        self.resultMessage = ko.computed(function () {
            if (!self.resultIsSuccess()) return i18n.t("app:language.changeStatusError");

            return self.originalStatus() === 'Active'
                ? i18n.t("app:language.deactivated")
                : i18n.t("app:language.activated");
        });

        self.title = ko.computed(function () {
            return self.originalStatus() === 'Active'
                ? i18n.t("app:language.deactivateTitle")
                : i18n.t("app:language.activateTitle");
        });
    ...
    customModal.prototype.changeStatus = function () {
        ...
        if (self.isValid()) {
            if (self.originalStatus() === 'Active') {
                ...
                url = config.adminApi('/Culture/Deactivate');
            } else {
                targetStatus = 'Active';
                url = config.adminApi('Culture/Activate');
            }
