...
IF DB_ID(N'__DATABASENAME__') IS NULL BEGIN
    DECLARE @cmd nvarchar(500)
    SET @cmd = N'CREATE DATABASE [__DATABASENAME__]'
    EXEC(@cmd)
END    
GO
...
SET @jobname = N'__DATABASENAME__' + '_Job_DeleteExpiredSessions' 
...
IF (@ver >= 8)
    SET @cmd = N'
        ...
        AS
            CREATE TABLE [__DATABASENAME__].dbo.ASPStateTempSessions (
            ...
            CREATE NONCLUSTERED INDEX Index_Expires ON [__DATABASENAME__].dbo.ASPStateTempSessions(Expires)

            CREATE TABLE [__DATABASENAME__].dbo.ASPStateTempApplications (
                AppId               int             NOT NULL PRIMARY KEY,
                AppName             char(280)       NOT NULL,
            ) 

            CREATE NONCLUSTERED INDEX Index_AppName ON [__DATABASENAME__].dbo.ASPStateTempApplications(AppName)
...
SET @cmd = N'
    ...
    SELECT @appId = AppId
    FROM [__DATABASENAME__].dbo.ASPStateTempApplications
    ...
    IF @appId IS NULL BEGIN
        ...
        SELECT @appId = AppId
        FROM [__DATABASENAME__].dbo.ASPStateTempApplications WITH (TABLOCKX)
        WHERE AppName = @appName
        ...
        BEGIN
            ...
            INSERT [__DATABASENAME__].dbo.ASPStateTempApplications
            VALUES
            (@appId, @appName)
...
IF (@ver >= 8)
    SET @cmd = N'
        ...
        AS
            ...
            UPDATE [__DATABASENAME__].dbo.ASPStateTempSessions
            ...
            IF @length IS NOT NULL BEGIN
                READTEXT [__DATABASENAME__].dbo.ASPStateTempSessions.SessionItemLong @textptr 0 @length
            END
...
IF (@ver >= 8)
    SET @cmd = N'
        ...
        AS    
            ...
            INSERT [__DATABASENAME__].dbo.ASPStateTempSessions 
                (SessionId, 
                 SessionItemShort, 
                 Timeout, 
                 Expires, 
...
SET @cmd = N'
    CREATE PROCEDURE dbo.TempRemoveStateItem
        ...
        @lockCookie int
    AS
        DELETE [__DATABASENAME__].dbo.ASPStateTempSessions
        WHERE SessionId = @id AND LockCookie = @lockCookie
        RETURN 0'
EXEC(@cmd)    
GO
    ...
    SET @cmd = N'
        ...
        AS
            ...
            INSERT #tblExpiredSessions (SessionId)
                SELECT SessionId
                FROM [__DATABASENAME__].dbo.ASPStateTempSessions WITH (READUNCOMMITTED)
                WHERE Expires < @now
            ...
            BEGIN 
                ...
                WHILE @@FETCH_STATUS = 0 
                    BEGIN
                        DELETE FROM [__DATABASENAME__].dbo.ASPStateTempSessions WHERE SessionId = @SessionId AND Expires < @now
                        FETCH NEXT FROM ExpiredSessionCursor INTO @SessionId
                    END
...
BEGIN TRANSACTION            
    ...
    -- Add the job
    SET @nameT = N'__DATABASENAME__' + '_Job_DeleteExpiredSessions'
    ...
    -- Add the job steps
    SET @nameT = N'__DATABASENAME__' + '_JobStep_DeleteExpiredSessions'
    EXECUTE @ReturnCode = msdb.dbo.sp_add_jobstep 
            @job_id = @JobID,
            @step_id = 1, 
            @step_name = @nameT, 
            @command = N'EXECUTE DeleteExpiredSessions', 
            @database_name = N'__DATABASENAME__', 
            @server = N'', 
            @subsystem = N'TSQL', 
    ...
    SET @nameT = N'__DATABASENAME__' + '_JobSchedule_DeleteExpiredSessions'
